...
df <- df %>%
  filter(grepl("Endeks", index_type), grepl("Transportation", type_eng)) %>%
  select(data_date, value) %>%
  rename(ulastirma = value)
...
ts_data <- ts(df$ulastirma, start = c(2016, 12), frequency = 12)

seasonal_adjust_ulastirma <- function(ts_data) {
  ...
  spec <- tramoseats_spec(
    ...
    usrdef.outliersDate = c("2023-08-01")
  )
...
ulastirma_sa <- seasonal_adjust_ulastirma(ts_data)



# Extract the series components safely
if (!is.null(ulastirma_sa$final$series)) {
  sa_series <- ulastirma_sa$final$series
  ...
  # Export to Excel
  write_xlsx(result_df, path = "ma_results/ulastirma_sa_series_alternatives.xlsx")
  
  # Print summary
  print("Seasonal adjustment completed successfully!")
  print("Results exported to 'ulastirma_sa_series.xlsx'")
  ...
  # Check for errors
  if (!is.null(ulastirma_sa$diagnostics)) {
    print(ulastirma_sa$diagnostics)
  }
}
